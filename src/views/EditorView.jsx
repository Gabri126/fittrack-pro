...
export default function EditorView({ library, setLibrary, history, setCurrentTab }) {
  ...
  const [route, setRoute] = useState('dashboard'); // 'dashboard', 'editor', 'plan-details'
  const [selectedPlanId, setSelectedPlanId] = useState(null);
  const [shareToast, setShareToast] = useState('');

  const handleSharePlan = async (plan) => {
    if (ctx?.session && ctx?.sharePlan) {
      const { slug, error } = await ctx.sharePlan(plan);
      if (!error && slug) {
        const url = `${window.location.origin}?p=${slug}`;
        if (navigator.clipboard) {
          await navigator.clipboard.writeText(url);
          setShareToast('Link copiato!');
        }
      } else {
        setShareToast('Errore condivisione');
      }
    } else {
      // Offline fallback: share via native share or clipboard
      const text = `Prova la mia scheda "${plan.name}" su FitTrack Ultra!`;
      if (navigator.share) {
        ...
        if (navigator.clipboard) {
          ...
          setShareToast('Piano copiato negli appunti!');
        }
      }
    }
    if (shareToast === '') setTimeout(() => setShareToast(''), 2500);
  };

  const [selectedDateTs, setSelectedDateTs] = useState(() => {
    const d = new Date();
    d.setHours(0,0,0,0);
    return d.getTime();
  });

  // Critical Fix: Define muscleStatus inside the component using real history and EXERCISE_DB
  const muscleStatus = useMemo(() => {
    const groups = ['Petto', 'Dorso', 'Gambe', 'Spalle', 'Braccia', 'Core'];
    const sevenDays = 7 * 24 * 60 * 60 * 1000;
    
    return groups.map(group => {
      const vol = (history || [])
        .filter(s => (Date.now() - s.date) < sevenDays)
        .reduce((acc, s) => {
          const sessionVol = s.exercises?.filter(ex => {
            const dbEntry = EXERCISE_DB.find(db => db.name === ex.exerciseName);
            return dbEntry?.tags?.some(tag => tag.toLowerCase() === group.toLowerCase());
          }).reduce((sum, ex) => {
            // Allineamento: Usiamo 'sets' che è lo standard salvato nello storico
            const workoutSets = ex.sets || [];
            return sum + (workoutSets.reduce((ss, set) => ss + ((parseFloat(set.weight) || 0) * (parseInt(set.reps) || 0)), 0) || 0);
          }, 0) || 0;
      ...
      return { 
        ...
        percentage: Math.min(100, (vol / 5000) * 100) 
      };
    });
  }, [history]);
  ...
  const timelineDates = useMemo(() => {
    ...
    // ISO Monday start: Monday is 1, Sunday is 0 -> mapped to -6
  ...
  const enterDayEdit = (dayData) => {
    ...
    console.log('Entering day edit for:', dayData.id);
    setRoute('day-edit');
  };
  ...
  if (route === 'dashboard') {
    return (
      <div className="flex flex-col h-full overflow-y-auto hide-scrollbar bg-black text-white pb-32">
        <header className="px-6 pt-12 pb-4 flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-black tracking-tighter uppercase italic text-white leading-none">Mission Control</h1>
            <p className="text-[10px] font-black text-muted uppercase tracking-[0.2em] mt-2">Status & Readiness</p>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-[10px] font-black text-muted uppercase tracking-widest mb-1">Allenamenti</span>
            <span className="text-xl font-black italic text-accentBlue">{history.length}</span>
          </div>
        </header>

        {/* Muscle Heatmap Section */}
        <section className="px-6 mb-8">
          <div className="bg-surface/40 border border-white/5 rounded-[32px] p-6 backdrop-blur-xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[10px] font-black text-white/50 uppercase tracking-widest">Stato Muscolare (7gg)</h3>
              ...
              {muscleStatus.map(muscle => (
                <div key={muscle.name} className="space-y-2">
                  <div className="flex justify-between items-center px-0.5">
                    <span className="text-[8px] font-black text-muted uppercase">{muscle.name}</span>
                  </div>
                  <div className="h-1.5 bg-white/5 rounded-full overflow-hidden">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${muscle.percentage}%` }}
                      className="h-full bg-accentBlue rounded-full shadow-[0_0_8px_rgba(10,132,255,0.4)]"
                    />
                  </div>
                </div>
              ))}
        ...
        <div className="w-full mb-8">
          <div className="grid grid-cols-7 gap-1 px-4">
            {timelineDates.map(ts => {
              ...
              return (
                <div key={ts} onClick={() => setSelectedDateTs(ts)} className="flex flex-col items-center cursor-pointer transition-transform active:scale-90">
                  <span className={cn("text-[8px] font-black uppercase tracking-wider mb-2", isSelected ? "text-white" : "text-muted")}>
                    {DAY_LABELS[data.dayOfWeek]}
                  </span>
                  <div className={cn(
                    "w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 relative", 
                    isSelected ? "bg-surface border-2 border-white/40 shadow-[0_0_15px_rgba(255,255,255,0.1)]" : "bg-transparent border border-transparent"
                  )}>
                    {data.isCompleted ? (
                      <div className="w-7 h-7 rounded-full bg-green-500 shadow-[0_0_10px_rgba(52,199,89,0.3)] flex items-center justify-center">
                        <CheckCircle2 size={14} className="text-black" />
                      </div>
                    ) : (data.isScheduled && !data.isPast) ? (
                      <div className={cn(
                        "w-7 h-7 rounded-full border-2 border-dashed border-accentOrange bg-accentOrange/10 flex items-center justify-center"
                      )}>
                        <span className="text-xs font-black text-accentOrange">{data.dayNum}</span>
                      </div>
                    ) : (
                      <div className="w-7 h-7 rounded-full bg-surface border border-border/50 flex items-center justify-center">
                         <span className={cn("text-xs font-bold", isSelected ? "text-white" : "text-muted")}>{data.dayNum}</span>
                      </div>
                    )}
                ...
                return (
                  <div key={plan.id} className="bg-surface/80 backdrop-blur-md border border-border/50 rounded-3xl p-5 flex flex-col space-y-4 group transition-all hover:border-border cursor-pointer" onClick={() => { setSelectedPlanId(plan.id); setIsWiggleMode(false); setRoute('plan-details'); }}>
                    ...
                    <div className="flex items-center space-x-2 pt-4 border-t border-border/20">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); handleSharePlan(plan); }} className="p-2.5 rounded-2xl text-muted bg-white/5 hover:text-accentBlue hover:bg-accentBlue/10 transition-colors"><Share2 size={16} /></button>
                    ...
                    return (
                      <div key={dayOfWeekIdx} className="relative flex flex-col items-center group">
                        ...
                        <motion.div
                          ...
                          onTap={() => {
                            if (!isWiggleMode) enterDayEdit(dayData);
                          }}
                          className={cn(
